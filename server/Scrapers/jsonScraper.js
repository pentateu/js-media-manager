...
var JSONScraper = module.exports = function(ScraperImpl) {
	...
	if(typeof ScraperImpl !== "function"){
		...
		throw "The parameter must be a constructor function and it must follow the Scraper spec!";
	}
	...
	context.search = function(retries){
		...
		if( ! scraperInstance.buildURL){
			p.reject(BUILD_URL_FUNC_NOT_SET);
			return p;
		}
		if( ! scraperInstance.processSearchResult){
			p.reject(PROCESS_SEARCH_RESULT_FUNC_NOT_SET);
			return p;
		}
		...
		http.get(url, function(res) {
			...
			res.on('close', function () {
				...
				if(jsonResult){
					...
					if(retries >= LIMIT_RETRIES){
						p.reject(ERROR_IMDB_CLOSE_RESPONSE);
					}
					else{
			...
			res.on('end', function () {
				...
				try{
					...
					p.reject(ERROR_JSON_PARSE.setCause(e));
				}
				...
				try{
					...
					p.reject(ERROR_PROCESS_SEARCH_RESULT.setCause(e));
				}
			});
		...
		.on('error', function(e) {
			...
			p.reject(ERROR_JSON_SERVICE.setCause(e));
		});
